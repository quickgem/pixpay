...
def generate_snap():
    ...
    for jsf in js_files:
        ...
        if fnbase.startswith("mod_"):
            ret = os.system(
                cmd + " -m " + jsf + " " + WD + "scripts/" + fnbase + ".snap"
            )
        else:
...
def write_output():
    ...
    # Write APP_RES_INFO
    bs = struct.pack(
        "32sI4sI16s16sBB14x256x32s16s32s",
        ...
        1 if RES_DIRS["scripts"] == "snap" else 0,  # isbin
...
def sign_output():
    ...
    if sys.platform == "win32":
        ...
        cmd = "./winmake/mp30signapp -s " + OUTF
    else:
...
def main():
    ...
    if len(sys.argv) == 2 and sys.argv[1] == "-b":
        # NB: this is also used as a condition when writing package header
