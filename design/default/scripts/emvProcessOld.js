...
ViewModel("emvProcess", {
  ...
  methods: {
    ...
    startEMV: function () {
      ...
      // Logs the start time for EMV processing
      console.log("startEMV start time ==========>", tret.data);

      // Initializes EMV callbacks
    ...
    jump2Next:function (args){
      console.log('emv_next ====> ', args)
      GLOBAL_JUMP("", args);
    },
      ...
      function cb() {
        ...
        if (ret.code >= 0) {
          ...
          if (ret.data.is_done > 0) {
            that.showPin = false; // displays pin
